...
UINT DoCalc_1(LPVOID pParam)
{
	if (pParam)
	{
		...
		///////// ////////////////////////////////////////////////
		if (m_Calc->OutPutFlags == 0 || !fopen_s(&ptrFile, "C:\\SParticle\\report.txt", "wt"))
		{
			int nParticle = 2;
			...
			if (m_Calc->OutPutFlags) m_Calc->Group->WriteState(ptrFile, m_Calc->Group->GetCurTime());

			m_Calc->Group->CalcRelations(0.0, NULL, NULL);
			...
			while ((i++ < nIter || m_Calc->Group->m_maxCountLimit > 5000) && (m_Calc->State & M_STATE_ON))
			{
				...
				{
					if (m_Calc->OutPutFlags) fprintf(ptrFile, "%d\t", i);
					...
					if (v_MaxSD.Accel3D != 0.0) m_Calc->Time.CurStep = m_Calc->Time.BaseStep / v_MaxSD.Accel3D; 
					...
					if (m_Calc->OutPutFlags) m_Calc->Group->WriteState(ptrFile, 1, 1);
				}
...
UINT DoCalc_Find(LPVOID pParam)
{
	if (pParam)
	{
...

		{
			...
			{
				char tmpCamtion[500];
				sprintf_s(tmpCamtion, "Find NP%d_C%d_ADD%d_Wind%d_DCT%d_ACT%d_DL%d_nc%0.3d_PT%g_%d", 
					m_Calc->curCalc.Group->GetCount(), m_Calc->curCalc.ClassicType, m_Calc->curCalc.AdditiveType, 
					m_Calc->curCalc.WindType,m_Calc->curCalc.DensityCalcType, m_Calc->curCalc.AccCalcType,m_Calc->curCalc.DensityDerivLevel, m_Calc->stat.Count,
			...
			for (m_Calc->stat.Count = 0; (m_Calc->FindState & M_STATE_ON); m_Calc->stat.Count++)
			{
				...
				if (m_Calc->curCalc.OutPutFlags == 0 || !fopen_s(&ptrFile, fileMane, "wt"))
				{
					...
					tmppos += sprintf_s(tmppos, 1000, "Iteration\t%d\tReported\t%d\n", m_Calc->stat.Count, m_Calc->stat.Reported);
					{
						for (int i = 0; i < m_Calc->curCalc.Group->GetCount(); i++)
						{
							...
							if (m_Calc->curCalc.ParentSpaceThickness < 1.0)
							{
								...
								while (bDo)
								{
									...
									for (int j = 0; j < i && !bDo; j++)
									{
										...
										if (CSpParticle::Get3D_Pwr1(cmpPos) + m_Calc->curCalc.ParentSpaceThickness < 1.0)
										{
							...
							double Speed3dPwr1 = CSpParticle::Get3D_Pwr1(initData.Speed);
					...
					if (m_Calc->curCalc.OutPutFlags) m_Calc->curCalc.Group->WriteState(ptrFile, m_Calc->curCalc.Group->GetCurTime());
					m_Calc->curCalc.Group->CalcRelations(0.0, NULL, NULL);
					...
					m_Calc->curCalc.Stat.Rel_Min = { 0 };
					m_Calc->curCalc.Stat.Rel_Max = { 0 };
					...
					m_Calc->curCalc.Stat.Prev_Rel_Min = {0};
					m_Calc->curCalc.Stat.Prev_Rel_Max = {0};
					...
					{
						{
							if (m_Calc->curCalc.OutPutFlags) fprintf(ptrFile, "%d\t", m_Calc->curCalc.CurStep);
							...
							if (m_Calc->curCalc.OutPutFlags) m_Calc->curCalc.Group->WriteState(ptrFile, 1, 1);
...
void PaintTraceToCDC(CDC* DCMem, SP_Calc* mData, int in_DrawFlags, SPos& delta00)
{
	if (DCMem && mData && mData->Group)
	{
		...
		if ((in_DrawFlags & SP_DRAW_XYZ) == SP_DRAW_XYZ)
		{
			...
			in_DrawFlags &= (0xFFFFFFFF ^ SP_DRAW_XYZ);

			mData->DrawSet.iShowFlags &= (0xFFFFFFFF ^ SP_SHOWFLAG_TEXT);


			mData->DrawSet.m_00_Pos.X = (mData->DrawSet.BMP_Size.X - (mData->DrawSet.PosMax.X - mData->DrawSet.PosMin.X) * mData->DrawSet.ViewMltpl) / 2.0 - oldSet.PosMin.X * mData->DrawSet.ViewMltpl;
			mData->DrawSet.m_00_Pos.Y = (mData->DrawSet.BMP_Size.Y - (mData->DrawSet.PosMax.Y - mData->DrawSet.PosMin.Y) * mData->DrawSet.ViewMltpl) / 2.0 - oldSet.PosMin.Y * mData->DrawSet.ViewMltpl;
			PaintTraceToCDC(DCMem, mData, in_DrawFlags | SP_DRAW_XY, delta00);

			//mData->DrawSet.m_00_Pos.X += mData->DrawSet.BMP_Size.X;

			mData->DrawSet.m_00_Pos.X = mData->DrawSet.BMP_Size.X + (mData->DrawSet.BMP_Size.X - (mData->DrawSet.PosMax.Z - mData->DrawSet.PosMin.Z) * mData->DrawSet.ViewMltpl) / 2.0 - oldSet.PosMin.Z * mData->DrawSet.ViewMltpl;
			mData->DrawSet.m_00_Pos.Y = (mData->DrawSet.BMP_Size.Y - (mData->DrawSet.PosMax.Y - mData->DrawSet.PosMin.Y) * mData->DrawSet.ViewMltpl) / 2.0 - oldSet.PosMin.Y * mData->DrawSet.ViewMltpl;
			...
			mData->DrawSet.m_00_Pos.X = (mData->DrawSet.BMP_Size.X - (mData->DrawSet.PosMax.X - mData->DrawSet.PosMin.X) * mData->DrawSet.ViewMltpl) / 2.0 - oldSet.PosMin.X * mData->DrawSet.ViewMltpl;
			mData->DrawSet.m_00_Pos.Y = mData->DrawSet.BMP_Size.Y + (mData->DrawSet.BMP_Size.Y - (mData->DrawSet.PosMax.Z - mData->DrawSet.PosMin.Z) * mData->DrawSet.ViewMltpl) / 2.0 - oldSet.PosMin.Z * mData->DrawSet.ViewMltpl;
			//mData->DrawSet.iShowFlags |= SP_SHOWFLAG_TEXT;
		...
		else
		{
			...
			if ((mData->DrawSet.iShowFlags & SP_SHOWFLAG_CLEARBKG))
			...
			try
			{
				...
				if (nTrace > 1 && mData->Group->m_Array[0] && mData->Group->m_Array[0]->m_Data)
				{
					...
					if ((mData->DrawSet.iShowFlags& SP_SHOWFLAG_TEXT))
					{
						char tmps[300];
					...
					for (int idPrt = 0; idPrt < nTrace; idPrt++)
					{
						...
						{
							nPoints = mData->Group->m_Array[idPrt]->m_Data->CurPos + 1;
							HPEN PenLink = CreatePen(PS_SOLID, 1, mRGB[(idPrt + 8) % 16]);
							SelectObject(hDC, PenLink);

							pData = mData->Group->m_Array[idPrt]->m_Data->m_Array;
							...
							{
								int posC1 = (int)(mData->DrawSet.m_00_Pos.X + delta00.X + (double)(((double*)(&pData[i].position))[idxCrd1] * mData->DrawSet.ViewMltpl));
								int posC2 = (int)(mData->DrawSet.m_00_Pos.Y + delta00.Y + (double)(((double*)(&pData[i].position))[idxCrd2] * mData->DrawSet.ViewMltpl));

								MoveToEx(hDC, posC1, posC2, NULL);

								for (int j = 0; j < nTrace; j++)
									if (i != j && ((1 << j) & mData->DrawSet.ShowLinksTo))
									{
										mData->Group->m_Array[j]->FindViewPoint(&VP_Data, pData + i, pData[i].TimePoint, NULL);

										LineTo(hDC, (int)(mData->DrawSet.m_00_Pos.X + delta00.X + (((double*)(&VP_Data.position))[idxCrd1] * mData->DrawSet.ViewMltpl)), (int)(mData->DrawSet.m_00_Pos.Y + delta00.Y + (((double*)(&VP_Data.position))[idxCrd2] * mData->DrawSet.ViewMltpl)));
										MoveToEx(hDC, posC1, posC2, NULL);
									}
							}
					...
					for (int idPrt = 0; idPrt < nTrace; idPrt++)
					{
						...
						if ((mData->DrawSet.ShowItems & Pwr2Id))
						{
							...
							int iStep = nPoints / 10000;
							if (iStep < 1) iStep = 1;
							SelectObject(hDC, PenPtr);

							int posC1 = (int)(mData->DrawSet.m_00_Pos.X + delta00.X + (double)(((double*)(&pData[0].position))[idxCrd1] * mData->DrawSet.ViewMltpl));
							int posC2 = (int)(mData->DrawSet.m_00_Pos.Y + delta00.Y + (double)(((double*)(&pData[0].position))[idxCrd2] * mData->DrawSet.ViewMltpl));
